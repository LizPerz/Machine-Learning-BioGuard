...
def calcular_baseline(
    ...
    hrv = _media([estimar_hrv(rec["pulso_bpm"], rec.get("sudoracion_gsr") or 0) for rec in reposo])
...
def _sueno_riesgo(lectura: dict, baseline: dict[str, float]) -> float:
    ...
    if temp_base > 0:
        ...
        if diff > 0.5:
            riesgo += diff * 20
    gsr = lectura.get("sudoracion_gsr") or 0
    if gsr > 5:
        riesgo += gsr * 5
...
def construir_features(
    ...
    gsr = [float(rec.get("sudoracion_gsr") or 0.0) for rec in lecturas]
    hrv = [
        float(rec.get("hrv"))
        if rec.get("hrv") is not None
        else estimar_hrv(rec["pulso_bpm"], rec.get("sudoracion_gsr") or 0)
        for rec in lecturas
    ]

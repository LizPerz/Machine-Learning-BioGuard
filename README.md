...
| `BIOGUARD_CORS_ORIGINS` | `[]` (ninguno) | Orígenes permitidos (deny-by-default) |
| `BIOGUARD_AUTH_DESHABILITADO` | `false` | **Solo `true` en entornos de prueba**: si es `false`, siempre se exige JWT |
| `BIOGUARD_DEBUG` | `false` | Modo debug |
...
La seguridad se aplica como código en cada capa (sin modificar la lógica de negocio):

| Capa | Herramienta | Qué valida |
|---|---|---|
| Pre-commit | ruff, bandit, gitleaks | Estilo, SAST y secretos antes del commit |
| CI (secret-scan) | Gitleaks | Fugas de secretos en el repo |
| CI (sast) | Bandit + Semgrep + CodeQL | Vulnerabilidades en el código (SAST) |
| CI (dependency-scan) | pip-audit + pip-licenses | CVEs en dependencias (SCA) y licencias |
| CI (sbom) | Syft/Anchore | Inventario de componentes (SBOM) |
| CI (container-security) | Trivy | CVEs en imagen Docker + misconfig de IaC |
| CI (dast) | OWASP ZAP | Vulnerabilidades dinámicas |
| Runtime | JWT servicio-a-servicio (exigido por defecto) + cabeceras OWASP + rate limiting + logs con enmascaramiento PII | Autenticación y endurecimiento HTTP |
| Dependencias | Dependabot | Actualizaciones de seguridad automáticas |

### Endurecimiento del runtime (por defecto seguro)

- **Autenticación exigida siempre** salvo `BIOGUARD_AUTH_DESHABILITADO=true` (solo
  tests). Si falta el secreto en producción, toda petición es rechazada (falla cerrado).
- **`/docs`, `/redoc` y `/openapi.json` deshabilitados en producción** para no
  exponer la superficie de la API.
- **CORS en deny-by-default** (`BIOGUARD_CORS_ORIGINS=[]`); configurar orígenes explícitos.
- **Logs con enmascaramiento PII/HIPAA**: JWT, secretos, `paciente_id` y la URI de
  Mongo (credenciales) se enmascaran antes de escribirse.

Pruebas de seguridad (pytest) en `tests/test_devsecops_security_controls.py`,
`tests/test_devsecops_authz_cors_rate.py` y `tests/test_devsecops_pipeline_config.py`.
Atajos locales en el `Makefile` (`make ci-local`). Ver `SECURITY.md` para la política de reporte.

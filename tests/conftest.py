...
def client(fake_mongo: FakeMongo):
    # Entorno de prueba explícito: se desactiva la autenticación para validar
    # la lógica de negocio sin infraestructura de JWT.
...
def client_con_db(fake_mongo: FakeMongo):
    s = Settings(mongo_uri="mongodb://fake", worker_sync_enabled=False, auth_deshabilitado=True)
    return TestClient(build_app(mongo=fake_mongo, settings_override=s))
